...
public class ShortBus extends BasicGame
{
	...
	{
		app = new AppGameContainer(new ShortBus("ShortBus"),800,600, false);
		app.start();
	}
	...
	private boolean blocked(double dx, double dy) {
		System.out.println(Math.round(dx-shortbus.getWidth()/(float)tileWidth));
		System.out.println((Math.round(dy-.125*shortbus.getWidth()/(float)tileWidth)));
		return blocked[(int)(Math.round(dx-shortbus.getWidth()/(float)tileWidth))][(int)(Math.round(dy-.125*shortbus.getWidth()/(float)tileWidth))];
	}
	...
	private boolean testPolygon(Polygon test){
		...
		if(blocked(test.getMaxX(),test.getMaxY())) return false;
		else if(blocked(test.getMaxX(),test.getMinY())) return false;
		else if(blocked(test.getMinX(), test.getMaxY())) return false;
		else if(blocked(test.getMinX(), test.getMinY())) return false;
		else	return true;
